...
(function() {
  ...
  SSH2Shell = (function(superClass) {
    ...
    SSH2Shell.prototype._processData = function(data) {
      this._buffer += data;
      if (this.command && this.command.indexOf(this.sshObj.localAuthCommand + " ") !== -1) {
        return this._processPasswordPrompt();
      } else if (this.command && this.command.indexOf(this.sshObj.remoteAuthCommand + " ") !== -1) {
        ...
        return this.sshObj.dataReceivedTimer = setTimeout((function(_this) {
          return function() {
            if (_this.command.length > 0 && _this.standardPromt.test(_this._buffer.replace(_this.command.substr(0, _this._buffer.length), ""))) {
            ...
            } else if (_this.command.length > 0) {
              if (_this.sshObj.debug) {
                _this.emit('msg', _this.sshObj.server.host + ": Command waiting");
              }
              _this.emit('commandProcessing', _this.command, _this._buffer, _this.sshObj, _this._stream);
            ...
            } else if (_this.command.length < 1 && _this.standardPromt.test(_this._buffer)) {
              if (_this.sshObj.debug) {
              ...
              if (_this.sshObj.debug) {
                _this.emit('msg', _this.sshObj.server.host + ": No command processing to first prompt");
              }
              return _this.emit('commandProcessing', _this.command, _this._buffer, _this.sshObj, _this._stream);
            }
          };
        })(this), 500);
      }
    ...
    SSH2Shell.prototype._processPasswordPrompt = function() {
      ...
      if (!this.sshObj.pwSent) {
        ...
        if (this.passwordPromt.test(response)) {
          if (this.sshObj.verbose) {
            this.emit('msg', (this.sshObj.server.host + ": ") + this.sshObj.localAuthCommand + (" password faied: response: " + response));
          }
          this.emit('error', this.sshObj.localAuthCommand + (" password was incorrect for " + this.sshObj.server.userName), this.sshObj.localAuthCommand + " authentication");
          if (this.sshObj.debug) {
    ...
    SSH2Shell.prototype._processSSHPrompt = function() {
      ...
      if (!this.sshObj.sshAuth) {
        if (this.passwordPromt.test(response)) {
          if (this.sshObj.debug) {
            this.emit('msg', (this.sshObj.server.host + ": ") + this.sshObj.remoteAuthCommand + " password prompt");
          }
          this.sshObj.sshAuth = true;
          return this._stream.write("" + this.sshObj.server.password + this.sshObj.enter);
        } else if (this.passphrasePromt.test(response)) {
          if (this.sshObj.debug) {
            this.emit('msg', (this.sshObj.server.host + ": ") + this.sshObj.remoteAuthCommand + " passphrase prompt");
          }
          this.sshObj.sshAuth = "true";
          return this._stream.write("" + this.sshObj.server.passPhrase + this.sshObj.enter);
        } else if (this.standardPromt.test(response)) {
          if (this.sshObj.debug) {
            this.emit('msg', (this.sshObj.server.host + ": ") + this.sshObj.remoteAuthCommand + " auth normal prompt");
          }
          this.sshObj.sshAuth = true;
        ...
        if ((password = this.passwordPromt.test(response) || this.passphrasePromt.test(response))) {
          ...
          this.emit('error', this.sshObj.remoteAuthCommand + (" authentication failed for " + this.sshObj.server.userName + "@" + this.sshObj.server.host), "Nested host authentication");
          if (this.sshObj.debug) {
    ...
    SSH2Shell.prototype._commandComplete = function() {
      ...
      if (this.command !== "" && this.command !== "exit" && this.command.indexOf(this.sshObj.remoteAuthCommand + " ") === -1) {
        if (this.sshObj.verbose) {
    ...
    SSH2Shell.prototype._loadDefaults = function() {
      ...
      if (!this.sshObj.localAuthCommand) {
        this.sshObj.localAuthCommand = "sudo";
      }
      if (!this.sshObj.remoteAuthCommand) {
        this.sshObj.remoteAuthCommand = "ssh";
      }
