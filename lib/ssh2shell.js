...
(function() {
  ...
  SSH2Shell = (function(superClass) {
    ...
    SSH2Shell.prototype._hosts = [];

    SSH2Shell.prototype._sshToNextHost = false;
    ...
    SSH2Shell.prototype._onData = function(data) {
      ...
      if (this.command.length > 0 && !this.standardPrompt.test(this._sanitizeResponse())) {
        ...
        this.idleTimer = setTimeout((function(_this) {
          return function() {
            return _this.emit('commandTimeout', _this.command, _this._buffer, _this._stream, _this.connection);
          };
        })(this), this.idleTime);
      ...
      return this.dataReceivedTimer = setTimeout((function(_this) {
        return function() {
          ...
          switch (true) {
            ...
            case _this.sshObj.sshToNextHost && _this.command.length > 0 && _this.command.indexOf("ssh ") === 0:
              if (_this.sshObj.debug) {
    ...
    SSH2Shell.prototype._processPasswordPrompt = function() {
      ...
      switch (true) {
        case passwordPrompt && !this.sshObj.server.password:
          if (this.sshObj.debug) {
            this.emit('msg', this.sshObj.server.host + ": Sudo password prompt no password set");
          }
    ...
    SSH2Shell.prototype._processSSHPrompt = function() {
      ...
      if (!this.sshObj.sshAuth) {
        ...
        if (passwordPrompt) {
          ...
          if (this.sshObj.verbose) {
            this.emit('msg', this.sshObj.server.host + ": SSH response: " + response);
          }
          if (this.sshObj.verbose && !this.sshObj.server.hasOwnProperty("password")) {
        ...
        if (standardPrompt) {
          ...
          if (this.sshObj.debug) {
            this.emit('msg', this.sshObj.server.host + ": SSH complete: next command");
          }
          this.sshObj.sshToNextHost = false;
    ...
    SSH2Shell.prototype._commandComplete = function() {
      ...
      if (this.command !== "" && this.command !== "exit" && this.sshObj.sshToNextHost !== true) {
        if (this.sshObj.verbose) {
    ...
    SSH2Shell.prototype._nextHost = function() {
      ...
      if (this.sshObj.debug) {
        this.emit('msg', this.sshObj.server.host + ": Remove previous event handlers");
      }
    ...
    SSH2Shell.prototype._nextPrimaryHost = function(connect) {
      if (this._hosts.length > 0) {
        if (this.sshObj.server) {
          ...
          this.emit('end', this._primaryhostSessionText, this.sshObj);
          this._removeListeners();
        }
        this.sshObj = this._hosts.shift();
        this._primaryhostSessionText = this.sshObj.server.host + ": ";
        if (this.sshObj.debug) {
          this.emit('msg', this.sshObj.server.host + ": Next primary host");
        }
        return this._initiate(connect);
      } else {
        if (this.sshObj.debug) {
    ...
    SSH2Shell.prototype._sshConnect = function() {
      ...
      this.sshObj.sshAuth = false;
      this.sshObj.sshToNextHost = true;
    ...
    SSH2Shell.prototype._runExit = function() {
      ...
      if (this.sshObj.exitCommands && this.sshObj.exitCommands.length > 0) {
        ...
        if (this._connections && this._connections.length > 0) {
          if (this.sshObj.verbose) {
            this.emit('msg', this.sshObj.server.host + ": Primary host: " + this._connections[0].server.host);
          }
    ...
    SSH2Shell.prototype._removeListeners = function() {
      if (this.sshObj.debug) {
        this.emit('msg', this.sshObj.server.host + ": Event handler count:");
        this.emit('msg', "keyboard-interactive: " + (this.listenerCount('keyboard-interactive')));
        this.emit('msg', "error: " + (this.listenerCount("error")));
        this.emit('msg', "data: " + (this.listenerCount("data")));
        this.emit('msg', "stderrData: " + (this.listenerCount("stderrData")));
        this.emit('msg', "end: " + (this.listenerCount('end')));
        this.emit('msg', "commandProcessing: " + (this.listenerCount('commandProcessing')));
        this.emit('msg', "commandComplete: " + (this.listenerCount('commandComplete')));
        this.emit('msg', "commandTimeout: " + (this.listenerCount('commandTimeout')));
        this.emit('msg', "msg: " + (this.listenerCount('msg')));
      }
      ...
      if (typeof this.sshObj.onError === 'function') {
        this.removeListener("error", this.sshObj.onError);
      }
      if (typeof this.sshObj.onStderrData === 'function') {
        this.removeListener("stderrData", this.sshObj.onStderrData);
      }
      if (typeof this.sshObj.onEnd === 'function') {
        this.removeListener("end", this.sshObj.onEnd);
      }
      if (typeof this.sshObj.onCommandProcessing === 'function') {
        this.removeListener("commandProcessing", this.sshObj.onCommandProcessing);
      }
      if (typeof this.sshObj.onCommandComplete === 'function') {
        this.removeListener("commandComplete", this.sshObj.onCommandComplete);
      }
      if (typeof this.sshObj.onCommandTimeout === 'function') {
        this.removeListener("commandTimeout", this.sshObj.onCommandTimeout);
      }
      if (this.idleTimer) {
        clearTimeout(this.idleTimer);
      }
    ...
    function SSH2Shell(hosts) {
      ...
      SSH2Shell.__super__.constructor.call(this, hosts);
      if (typeIsArray(hosts) && hosts.length > 0) {
        this._hosts = hosts;
      } else {
        this._hosts = [hosts];
      }
      ...
      this.on("error", (function(_this) {
        return function(err, type, close, callback) {
          ...
          if (_this.sshObj.debug) {
            _this.emit('msg', "Class.error: " + err + ", " + type);
          }
          ...
          if (close) {
            return _this._stream.close();
          }
        };
      })(this));
      if (this._hosts[0].server.tryKeyboard) {
    ...
    SSH2Shell.prototype._initiate = function(action) {
      if (this.sshObj.debug) {
    ...
    SSH2Shell.prototype._loadDefaults = function() {
      ...
      if (this.sshObj.msg) {
        ...
        if (this.sshObj.msg.send && typeof this.sshObj.msg.send === 'function') {
          this.on("msg", this.sshObj.msg.send);
        } else if (this.sshObj.msg && typeof this.sshObj.msg === 'function') {
      ...
      this.sshObj.onCommandTimeout = (ref6 = this.sshObj.onCommandTimeout) != null ? ref6 : (function(_this) {
        return function(command, response, stream, connection) {
          ...
          _this.emit("error", _this.sshObj.server.host + ": Command timed out after " + (_this.idleTime / 1000) + " seconds", "Timeout", true, function(err, type) {});
      ...
      if (this.sshObj.verbose) {
        return this.emit('msg', this.sshObj);
      }
    };
    ...
    SSH2Shell.prototype._connect = function() {
        ...
        return function() {
          ...
          return _this.connection.shell(_this.sshObj.window, {
            ...
            _this._stream.on("finish", function() {
              ...
              if (_this._hosts.length === 0) {
                return _this.emit('end', _this._allSessions, _this.sshObj);
              }
            });
      ...
      this.connection.on("close", (function(_this) {
        return function(had_error) {
          ...
          if (_this.dataReceivedTimer) {
            clearTimeout(_this.dataReceivedTimer);
          }
          if (had_error) {
          ...
          if (_this._hosts.length === 0) {
            if (typeof _this._callback === 'function') {
              _this._callback(_this._allSessions);
