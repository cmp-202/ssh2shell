...
(function() {
  ...
  SSH2Shell = (function(superClass) {
    ...
    SSH2Shell.prototype._setCommandTimer = function() {};
    ...
    SSH2Shell.prototype._processPasswordPrompt = function() {
      if (!this.sshObj.pwSent) {
        if (this.passwordPromt.test(this._buffer)) {
          ...
          if (this.sshObj.debug) {
            this.emit('msg', this.sshObj.server.host + ": Standard prompt after password sent");
          }
          return this._processNextCommand();
        }
      } else if (this.standardPromt.test(this._buffer)) {
        if (this.sshObj.debug) {
          this.emit('msg', this.sshObj.server.host + ": Standard prompt detected");
        }
        return this._processNextCommand();
      } else {
        if (this.passwordPromt.test(this._buffer)) {
    ...
    SSH2Shell.prototype._processNotifications = function() {
      ...
      while (this.command && ((sessionNote = this.command.match(/^`(.*)`$/)) || (msgNote = this.command.match(/^msg:(.*)$/)))) {
        if (sessionNote) {
          this.sshObj.sessionText += this.sshObj.server.host + ": " + sessionNote[1] + this.sshObj.enter;
    ...
    SSH2Shell.prototype._processNextCommand = function() {
      ...
      if (this.sshObj.verbose) {
        this.emit('msg', this.sshObj.server.host + ":command: " + this.command + this.sshObj.enter + "response: " + this._buffer);
      }
      this._buffer = "";
      if (this.sshObj.commands.length > 0) {
    ...
    SSH2Shell.prototype._nextHost = function() {
      ...
      this.sshObj.commands.unshift(this.sshObj.server.host + ": ssh -oStrictHostKeyChecking=no " + this.sshObj.server.userName);
      return this._processNextCommand();
    };
    ...
    SSH2Shell.prototype._runExit = function() {
      if (this.sshObj.exitCommands && this.sshObj.exitCommands.length > 0) {
        ...
        if (this.sshObj.debug) {
          this.emit('msg', this.sshObj.enter + "Parked hosts:");
        }
        if (this.sshObj.debug) {
        ...
        this.sshObj = this._connections.pop();
        if (this.sshObj.debug) {
          this.emit('msg', this.sshObj.server.host + ": Loaded previous host object");
        }
        if (this._connections.length > 0) {
          this.sshObj.exitCommands.push("exit");
        }
        ...
        if (this.sshObj.debug) {
          this.emit('msg', this.sshObj.server.host + ": Exit and close connection");
        }
        this.command = "exit";
        return this._stream.end("exit" + this.sshObj.enter);
      }
    };
    ...
    SSH2Shell.prototype._loadDefaults = function() {
      ...
      if (!this.sshObj.textColorFilter) {
        this.sshObj.textColorFilter = "(\x1b\[[0-9;]*m)";
      }
    ...
    function SSH2Shell(sshObj1) {
      ...
      this._setCommandTimer = bind(this._setCommandTimer, this);
      this._loadDefaults();
      ...
      this.on("keyboard-interactive", (function(_this) {
        return function(name, instructions, instructionsLang, prompts, finish) {
          if (_this.sshObj.debug) {
            return _this.emit('msg', _this.sshObj.server.host + ": this.onKeyboardInteractive");
          }
        };
      })(this));
      this.on("connect", (function(_this) {
        return function() {
          var ref;
          return _this.emit('msg', (ref = _this.sshObj.connectedMessage) != null ? ref : "Connected");
        };
      })(this));
      this.on("ready", (function(_this) {
        return function() {
          var ref;
          return _this.emit('msg', (ref = _this.sshObj.readyMessage) != null ? ref : "Ready");
        };
      })(this));
      ...
      this.on('commandProcessing', (ref1 = this.sshObj.onCommandProcessing) != null ? ref1 : (function(_this) {
        return function(command, response, sshObj, stream) {};
      })(this));
      this.on('commandComplete', (ref2 = this.sshObj.onCommandComplete) != null ? ref2 : (function(_this) {
        return function(command, response, sshObj) {
          if (_this.sshObj.debug) {
            return _this.emit('msg', _this.sshObj.server.host + ": this.onCommandComplete");
          }
        };
      })(this));
      this.on('commandTimeout', (ref3 = this.sshObj.onCommandTimeout) != null ? ref3 : (function(_this) {
        return function(command, response, stream, connection) {
          if (_this.sshObj.debug) {
            _this.emit('msg', _this.sshObj.server.host + ": this.onCommandTimeout");
          }
      ...
      this.on('end', (ref4 = this.sshObj.onEnd) != null ? ref4 : (function(_this) {
        return function(sessionText, sshObj) {
          if (_this.sshObj.debug) {
            return _this.emit('msg', _this.sshObj.server.host + ": this.onEnd");
          }
        };
      })(this));
      this.on("close", (function(_this) {
        return function(had_error) {
          if (_this.sshObj.debug) {
            _this.emit('msg', _this.sshObj.server.host + ": this.onClose");
          }
          if (had_error) {
      ...
      this.on("error", (ref5 = this.sshObj.onError) != null ? ref5 : (function(_this) {
        return function(err, type, close, callback) {
          if (close == null) {
            close = false;
          }
          if (_this.sshObj.debug) {
            _this.emit('msg', _this.sshObj.server.host + ": this.onError");
          }
      ...
      this.on("stderr", (function(_this) {
        return function(data, type) {
          return _this.emit('msg', "stdError: " + type + ", data: " + data);
        };
      })(this));
    }
      ...
      if (this.sshObj.server && this.sshObj.commands) {
        try {
          this.connection.on("keyboard-interactive", (function(_this) {
            return function(name, instructions, instructionsLang, prompts, finish) {
              if (_this.sshObj.debug) {
                _this.emit('msg', _this.sshObj.server.host + ": Connection.onKeyboardInteractive");
              }
            ...
            return function() {
              ...
              return _this.connection.shell({
                ...
                _this._stream.on("error", function(err) {
                  if (_this.sshObj.debug) {
                    _this.emit('msg', _this.sshObj.server.host + ": Stream.onError");
                  }
                ...
                _this._stream.stderr.on('data', function(data) {
                  ...
                  err.level = "stderr";
                  if (_this.sshObj.debug) {
                    _this.emit('msg', _this.sshObj.server.host + ": stderr.onData");
                  }
                ...
                _this._stream.on("readable", function() {
                  ...
                  try {
                    ...
                    return _this.emit('error', err, "Data processing", true);
                  }
                });
                _this._stream.on("finish", function() {
                  if (_this.sshObj.debug) {
                    _this.emit('msg', _this.sshObj.server.host + ": Stream.onFinish");
                  }
                  return _this.emit('end', _this.sshObj.sessionText, _this.sshObj);
                });
                return _this._stream.on("close", function(code, signal) {
                  if (_this.sshObj.debug) {
                    _this.emit('msg', _this.sshObj.server.host + ": Stream.onClose");
                  }
                  return _this.connection.end();
                });
              });
          ...
          this.connection.on("error", (function(_this) {
            return function(err) {
              if (_this.sshObj.debug) {
                _this.emit('msg', _this.sshObj.server.host + ": Connection.onError");
              }
          ...
          this.connection.on("close", (function(_this) {
            return function(had_error) {
              if (_this.sshObj.debug) {
                _this.emit('msg', _this.sshObj.server.host + ": Connection.onClose");
              }
