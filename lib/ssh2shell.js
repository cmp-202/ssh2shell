  ...
  SSH2Shell = (function(superClass) {
    ...
    function SSH2Shell(sshObj1) {
      ...
      this.on("keyboard-interactive", (function(_this) {
        return function(name, instructions, instructionsLang, prompts, finish) {
          if (_this.sshObj.keyboard - interactive) {
            return _this.sshObj.keyboard - interactive(name, instructions, instructionsLang, prompts, finish);
          }
        };
      })(this));
    ...
    SSH2Shell.prototype.connect = function() {
      ...
      if (this.sshObj.server && this.sshObj.commands) {
        try {
          this.conneciton.on("keyboard-interactive", (function(_this) {
            return function(name, instructions, instructionsLang, prompts, finish) {
          ...
          return this.connection.connect({
            ...
            hostVerifier: (ref = this.sshObj.server.hostVerifier) != null ? ref : (function(_this) {
              return function(hashedKey) {
                var clientKey, serverKey;
                clientKey = _this.sshObj.server.hashKey.replace(/[:]/g, "").toLowerCase();
                serverKey = hashedKey.replace(/[:]/g, "").toLowerCase();
                if (clientKey === "") {
                  if (_this.sshObj.verbose) {
                    _this.emit('msg', (_this.sshObj.server.host + " verbose: Server hash:") + hashedKey);
                  }
                  _this.sshObj.server.hashKey = hashedKey;
                  return true;
                } else if (serverKey === clientKey) {
                  if (_this.sshObj.debug) {
                    _this.emit('msg', _this.sshObj.server.host + ": Fingerprint verification passed");
                  }
                  return true;
                }
                if (_this.sshObj.verbose) {
                  _this.emit("msg", (_this.sshObj.server.host + " verbose: Hash values: Server = ") + serverKey + " <> Client = " + clientKey);
                }
                return false;
              };
            })(this),
