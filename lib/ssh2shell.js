  ...
  SSH2Shell = (function(superClass) {
    ...
    SSH2Shell.prototype._processData = function(data) {
      ...
      if (this.command && this.command.indexOf("sudo ") !== -1) {
        ...
        return this.sshObj.idleTimer = setTimeout(this._timedout, this._idleTime);
      }
    };
    ...
    SSH2Shell.prototype._processNextCommand = function() {
      ...
      if (this.sshObj.verbose) {
        this.emit('msg', this.sshObj.server.host + ": " + this._buffer);
      }
      this._buffer = "";
      if (this.sshObj.commands.length > 0) {
    ...
    SSH2Shell.prototype._loadDefaults = function() {
      ...
      this._idleTime = (ref1 = this.sshObj.idleTimeOut) != null ? ref1 : 5000;
    ...
    function SSH2Shell(sshObj1) {
      ...
      this.on('commandProcessing', (function(_this) {
        return function(command, response, sshObj, stream) {
          if (_this.sshObj.onCommandProcessing) {
            return _this.sshObj.onCommandProcessing(command, response, sshObj, stream, _this);
          }
        };
      })(this));
      this.on('commandComplete', (function(_this) {
        return function(command, response, sshObj) {
          if (_this.sshObj.debug) {
            _this.emit('msg', _this.sshObj.server.host + ": this.onCommandComplete command: " + command + ", response: " + response);
          }
          if (_this.sshObj.onCommandComplete) {
            return _this.sshObj.onCommandComplete(command, response, sshObj, _this);
          }
        };
      })(this));
      this.on('commandTimeout', (function(_this) {
        return function(command, response, stream, connection) {
          if (_this.sshObj.debug) {
            _this.emit('msg', _this.sshObj.server.host + ": this.onCommandTimeout command: " + command + ", response: " + response);
          }
          if (_this.sshObj.onCommandTimeout) {
            return _this.sshObj.onCommandTimeout(command, response, stream, connection, _this);
          } else {
      ...
      this.on('end', (function(_this) {
        return function(sessionText, sshObj) {
          if (_this.sshObj.debug) {
            _this.emit('msg', _this.sshObj.server.host + ": this.onEnd");
          }
          if (_this.sshObj.onEnd) {
            return _this.sshObj.onEnd(sessionText, sshObj, _this);
          }
        };
      })(this));
      ...
      this.on("error", (function(_this) {
        return function(err, type, close, callback) {
          ...
          if (_this.sshObj.onError) {
            _this.sshObj.onError(err, type, close, callback, _this);
            if (close) {
          ...
          this.connection.on("ready", (function(_this) {
            return function() {
              ...
              return _this.connection.shell({
                ...
                if (_this.sshObj.debug) {
                  _this.emit('msg', _this.sshObj.server.host + ": Ready Connection.shell");
                }
