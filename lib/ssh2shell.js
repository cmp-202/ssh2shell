...
(function() {
  ...
  SSH2Shell = (function(superClass) {
    ...
    SSH2Shell.prototype._loadDefaults = function() {
      ...
      this.sshObj.sshAuth = false;
      this.sshObj.server.hashInit = (ref = this.sshObj.server.hashInit) != null ? ref : false;
      ...
      if (this.sshObj.server && this.sshObj.commands) {
        try {
          ...
          return this.connection.connect({
            ...
            hostVerifier: (function(_this) {
              return function(hashedKey) {
                var hashKey;
                hashKey = _this.sshObj.server.hashKey.replace(/[:]/g, "").toLowerCase();
                ...
                if (hashKey === "") {
                  ...
                  return true;
                } else if (hashedKey === hashKey) {
                  if (_this.sshObj.debug) {
                    _this.emit('msg', _this.sshObj.server.host + " verbose: Fingerprint match passed");
                  }
                  return true;
                }
                _this.emit("error", "Server hash:" + hashedKey + " <> Client Hash:" + hashKey, "Failed fingerprint", false);
                return false;
              };
            })(this),
            hostHash: (ref2 = this.sshObj.server.hashMethod) != null ? ref2 : "md5"
          });
        } catch (error) {
          e = error;
