  ...
  SSH2Shell = (function(superClass) {
    ...
    SSH2Shell.prototype._processPasswordPrompt = function() {
      ...
      if (this.sshObj.verbose) {
        this.emit('msg', this.sshObj.server.host + ": Password sent: " + this.sshObj.pwSent);
      }
    ...
    SSH2Shell.prototype._processSSHPrompt = function() {
      ...
      standardPrompt = this.standardPrompt.test(response);
      if (this.sshObj.verbose) {
        this.emit('msg', this.sshObj.server.host + ": Password sent: " + this.sshObj.sshAuth + ",\n\tPrompt regex: " + this.passwordPromt);
      }
      if (this.sshObj.verbose) {
        this.emit('msg', this.sshObj.server.host + ": Response: |" + response + "|");
      }
      if (this.sshObj.verbose) {
        this.emit('msg', this.sshObj.server.host + ": Password prompt: " + passwordPrompt + ",\n\t Passphrase prompt: " + passphrasePrompt + ",\n\tStandard prompt: " + standardPrompt);
      }
      if (!this.sshObj.sshAuth) {
        ...
        if (passwordPrompt) {
          ...
          if (this.sshObj.debug) {
            this.emit('msg', this.sshObj.server.host + ": SSH standard prompt");
          }
          this.sshObj.sshAuth = true;
